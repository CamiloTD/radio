...
import { TYPE_CALL, TYPE_FILEHASH } from "../../utils/types";
import { ERC20Stego } from "../stego/erc20";
import { sha } from "../../utils/aes";
...
const WriteOperations = [ "lpop", "lpush", "set", "del", "hset", "hdel", "incr", "incrby", "hincrby", "linsert", "lrem", "lset", "ltrim" ];
const ReadOperations = [ "hget", "get", "lindex", "lrange", "lindex", "llen", "lpos"]
...
export async function Contract (stego: ERC20Stego, txId: string) {
    ...
    const localId = sha(stego.id + ":" + txId)
    ...
    async function initDb () {
        ...
        for (const call of calls) {
            const sandbox = { _id: localId };
    
            WriteOperations.forEach(name => sandbox[name] = function (...data) {
                // operations.push([name, ...data]);
                return db.redis[name](...data)
            });
    
            ReadOperations.forEach(name => {
